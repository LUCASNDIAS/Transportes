...
$(document).ready(function () {
    ...
    function enviarXml(id) {
        ...
        var etapa = 'enviar';
        var url = '/Transportes/backend/web/cte/default/cancel-' + etapa + '-xml/?id=' + id + '&motivo=' + motivo;
        ...
        if (motivo.length < 15 || motivo.length > 254) {
            // Mostra que houve erro na Barra
            erroBarra(etapa);
            mudaTexto(etapa, 'Motivo deve ter entre 15 e 255 caracteres.');
        }
    ...
    function reciboXml(id, recibo, repetir = false) {
        var etapa = 'recibo';
        var url = '/Transportes/backend/web/cte/default/' + etapa + '-xml/?id=' + id + '&recibo=' + recibo;
        ...
        $.get(url, function (data) {
            ...
            if (data.cStat == '105') {
                ...
                // Lote não Localizado
                var txtreenvia = data.xMotivo + '. <br />Reenviando CT-e.';
    ...
    function novoProtocolo(id) {
        ...
        $.get(url, function (data) {
            ...
            if (data.status === true) {
                ...
                var sucesso = 'CT-e Cancelado e protocolado.<br />Arquivo: ' + data.filename;
    ...
    function GetXml(id) {
        ...
        var url = '/Transportes/backend/web/cte/default/' + 'get' + '-xml/?id=' + id;
    ...
    function gerarPdf(id) {
        var etapa = 'pdf';
        var url = '/Transportes/backend/web/cte/default/gerar-' + etapa + '/?id=' + id;
