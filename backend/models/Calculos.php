...
class Calculos extends \yii\db\ActiveRecord
{
	...
	public function calculaFrete($tipo = 'db', $dados = array())
	{
		...
		// Calculos da Tabela
		$tabela_fretevalor = round($dadosTabela['fretevalor'] * $valornotas / 100,2);
		...
		$this->calculo['peso'] = ($peso=='') ? '0.00' : number_format($peso,2,'.','');
		$this->calculo['pesoexcedente'] = ($pesoexcedente=='') ? '0.00' : number_format($pesoexcedente,2,'.','');
		$this->calculo['valorminimo'] = ($valorminimo=='') ? '0.00' : number_format($valorminimo,2,'.','');
		$this->calculo['valorexcedente'] = ($valorexcedente=='') ? '0.00' : number_format($valorexcedente,2,'.','');
		$this->calculo['taxaextra'] = ($taxaextra=='') ? '0.00' : number_format($taxaextra,2,'.','');
		$this->calculo['desconto'] = ($desconto=='') ? '0.00' : number_format($desconto,2,'.','');
		$this->calculo['valornotas'] = ($valornotas=='') ? '0.00' : number_format($valornotas,2,'.','');
		$this->calculo['totalvolumes'] = $totalvolumes;
		$this->calculo['tabela_fretevalor'] = ($tabela_fretevalor=='') ? '0.00' : number_format($tabela_fretevalor,2,'.','');
		$this->calculo['tabela_despacho'] = ($tabela_despacho=='') ? '0.00' : number_format($tabela_despacho,2,'.','');
		$this->calculo['tabela_seccat'] = ($tabela_seccat=='') ? '0.00' : number_format($tabela_seccat,2,'.','');
		$this->calculo['tabela_itr'] = ($tabela_itr=='') ? '0.00' : number_format($tabela_itr,2,'.','');
		$this->calculo['tabela_gris'] = ($tabela_gris=='') ? '0.00' : number_format($tabela_gris,2,'.','');
		$this->calculo['tabela_pedagio'] = ($tabela_pedagio=='') ? '0.00' : number_format($tabela_pedagio,2,'.','');
		$this->calculo['tabela_outros'] = ($tabela_outros=='') ? '0.00' : number_format($tabela_outros,2,'.','');
		$this->calculo['fretetotal'] = ($fretetotal=='') ? '0.00' : number_format($fretetotal,2,'.','');
