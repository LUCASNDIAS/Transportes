...
class Calculos extends \yii\db\ActiveRecord
{
     ...
     * @return Ambigous <multitype:, string>|Ambigous <multitype:, string, unknown>
     */
    public function calculaFretecot($tipo = 'db', $dados = array())
    {
        // Verifica se há dados no array
        if (empty($dados)) {
            $this->msg[] = 'Dados não informados';
        ...
        if ($tipo == 'ajax') {
            ...
            foreach ($divide as $separa) {
                $campos     = explode('=', $separa);
                $key        = str_replace('Cotacao', '', $campos[0]);
                $key        = str_replace('%5B', '', $key);
                $key        = str_replace('%5D', '', $key);
                $novo[$key] = $campos[1];
            }
            ...
            // Array com os dados: seja modelo ou formulário!
            $p = (!isset($dados['Cotacao'])) ? $dados : $dados['Cotacao'];
        }
     ...
     * @return Ambigous <multitype:, string>|Ambigous <multitype:, string, unknown>
     */
    public function calculaFreteoc($tipo = 'db', $dados = array())
    {
        // Verifica se há dados no array
        if (empty($dados)) {
            $this->msg[] = 'Dados não informados';
        ...
        if ($tipo == 'ajax') {
            ...
            foreach ($divide as $separa) {
                $campos     = explode('=', $separa);
                $key        = str_replace('OrdemColeta', '', $campos[0]);
                $key        = str_replace('%5B', '', $key);
                $key        = str_replace('%5D', '', $key);
                $novo[$key] = $campos[1];
            }
            ...
            // Array com os dados: seja modelo ou formulário!
            $p = (!isset($dados['OrdemColeta'])) ? $dados : $dados['OrdemColeta'];
        }
